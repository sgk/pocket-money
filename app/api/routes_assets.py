...
def create_asset(payload: AssetCreate, user=Depends(get_current_user)):
    return assets_service.create_asset(user.uid, payload)


@router.patch("/{asset_id}", response_model=AssetOut)
def update_asset(asset_id: str, payload: AssetUpdate, user=Depends(get_current_user)):
    return assets_service.update_asset(user.uid, asset_id, payload)


@router.delete("/{asset_id}", response_model=AssetOut)
def delete_asset(asset_id: str, user=Depends(get_current_user)):
